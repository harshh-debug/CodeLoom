...
const ProblemPage = () => {
    ...
    const fetchProblem = async () => {
      ...
      try {
        ...
        if (!matchingStartCode) {
          console.error(
            "No matching start code found for language:",
  ...
  return (
    <div className="h-screen bg-zinc-900 flex overflow-hidden">
      ...
      {/* Left Panel */}
      <div className="w-1/2 flex flex-col border-r border-zinc-700/50 relative z-10 bg-zinc-900">
        {/* Left Tabs */}
        <div className="bg-zinc-800/50 border-b border-zinc-700/50 backdrop-blur-sm flex-shrink-0">
          <div className="flex">
            {[
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        {problem.visibleTestCases.map((example, index) => (
                          <div key={index} className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg p-4 backdrop-blur-sm">
                            ...
                            <div className="space-y-2 text-sm font-mono">
                              <div className="bg-zinc-900/50 p-3 rounded">
                                ...
                                <span className="text-zinc-300 ml-2">{example.input}</span>
                              </div>
                              <div className="bg-zinc-900/50 p-3 rounded">
                                <span className="text-emerald-400 font-semibold">Output:</span>
                                <span className="text-zinc-300 ml-2">{example.output}</span>
                              </div>
                              <div className="bg-zinc-900/50 p-3 rounded">
                                <span className="text-yellow-400 font-semibold">Explanation:</span>
                                <span className="text-zinc-300 ml-2">{example.explanation}</span>
                              </div>
                            </div>
                          </div>
                        ))}
              ...
              {activeLeftTab === "solutions" && (
                <div className="p-6 space-y-6">
                  ...
                  {problem.refrenceSolution?.map((solution, index) => (
                    <div key={index} className="bg-zinc-800/50 border border-zinc-700/50 rounded-lg backdrop-blur-sm overflow-hidden">
      ...
      {/* Right Panel */}
      <div className="w-1/2 flex flex-col relative z-10 bg-zinc-900">
        {/* Right Tabs */}
        <div className="bg-zinc-800/50 border-b border-zinc-700/50 backdrop-blur-sm flex-shrink-0">
          <div className="flex">
            {[
                    ...
                    {runResult.success ? (
                      <div className="space-y-4">
                        ...
                        <div className="flex gap-4 text-sm">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-zinc-300">Runtime: {runResult.runtime} sec</span>
                          </div>
                          <div className="flex items-center gap-1">
                            <HardDrive className="w-4 h-4 text-zinc-400" />
                            <span className="text-zinc-300">Memory: {runResult.memory} KB</span>
                          </div>
                        </div>
                        <div className="space-y-2">
                          {runResult.testCases?.map((tc, i) => (
                            <div key={i} className="bg-zinc-800/50 border border-zinc-700/50 rounded p-3 text-sm font-mono space-y-1">
                              <div><span className="text-indigo-400">Input:</span> <span className="text-zinc-300">{tc.stdin}</span></div>
                              <div><span className="text-emerald-400">Expected:</span> <span className="text-zinc-300">{tc.expected_output}</span></div>
                              <div><span className="text-blue-400">Output:</span> <span className="text-zinc-300">{tc.stdout}</span></div>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2">
                          ...
                          <h4 className="font-bold text-red-400">Test Failed</h4>
                        </div>
                        <div className="space-y-2">
                          {runResult.testCases?.map((tc, i) => (
                            <div key={i} className="bg-zinc-800/50 border border-zinc-700/50 rounded p-3 text-sm font-mono space-y-1">
                    ...
                    {submitResult.accepted ? (
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-2 text-sm">
                          <div><span className="text-zinc-400">Test Cases:</span> <span className="text-white">{submitResult.passedTestCases}/{submitResult.totalTestCases}</span></div>
                          <div><span className="text-zinc-400">Runtime:</span> <span className="text-white">{submitResult.runtime} sec</span></div>
                          <div><span className="text-zinc-400">Memory:</span> <span className="text-white">{submitResult.memory} KB</span></div>
                        </div>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        ...
                        <div className="text-sm">
                          <div><span className="text-zinc-400">Test Cases Passed:</span> <span className="text-white">{submitResult.passedTestCases}/{submitResult.totalTestCases}</span></div>
                        </div>
                      </div>
                    )}
                  </div>
